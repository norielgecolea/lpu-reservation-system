...
import { NexusApprovedEvent, NexusEquipmentItem, ReservedDateSlot, TIME_SLOTS } from './nexus-reservation.models';
...
@Component({
  selector: 'app-nexus-reservation',
  imports: [RouterLink, UiIcon, NexusStepper, ExternalEventNoticeStep],
  ...
  template: `
        ...
        <div class="bg-primary bg-[linear-gradient(135deg,#7a2342,#5f1830_55%,#8d2546)] text-white shadow-lg shrink-0">
          <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div class="flex items-center gap-3 flex-1">
              ...
              <div>
                <h1 class="text-xl sm:text-2xl font-black tracking-tight leading-tight">Nexus Room</h1>
                @if (!adminMode()) {
                  <p class="text-white/60 text-xs">LPU Laguna Nexus Room Reservation</p>
                }
              </div>
            </div>
    ...
    @if (view() === 'timeslots') {
      ...
      >
        <div class="bg-primary bg-[linear-gradient(135deg,#7a2342,#5f1830_55%,#8d2546)] text-white shadow-lg shrink-0">
          <div
            class="max-w-screen-2xl mx-auto px-4 sm:px-6 flex items-center gap-2 sm:gap-4"
            [class.py-3]="adminMode()"
            [class.py-4]="!adminMode()"
          >
            <button type="button" (click)="view.set('calendar')"
              class="flex shrink-0 items-center gap-1 text-white/70 hover:text-white transition-colors cursor-pointer text-sm">
            ...
            <div class="flex-1 min-w-0 text-center">
              ...
              <p class="text-white/60 text-xs hidden sm:block">Nexus Room — Select start and end time</p>
            </div>
            @if (adminMode()) {
              <a
                [routerLink]="returnPath()"
        ...
        <div
          class="max-w-screen-md mx-auto w-full px-4 sm:px-6 flex min-h-0 flex-1 flex-col pt-3 sm:pt-6"
        >
    ...
    @if (view() === 'form') {
      ...
      >
        <div class="bg-primary bg-[linear-gradient(135deg,#7a2342,#5f1830_55%,#8d2546)] text-white shrink-0 px-4 sm:px-6 py-3 flex items-center gap-4 shadow">
          ...
          <span class="font-bold text-sm tracking-wide">Nexus Room — Reservation Form</span>
...
export class NexusReservation implements OnInit {
  private readonly service = inject(NexusReservationService);
  ...
  readonly advanceDays = computed(() =>
    this.adminMode() ? 0 : RESERVATION_ADVANCE_DAYS.NEXUS,
  );
  ...
  readonly approvedEvents = signal<NexusApprovedEvent[]>([]);
  readonly availableEquipment = signal<NexusEquipmentItem[]>([]);
  ...
  ngOnInit(): void {
    ...
    this.maintSvc.getPublicBlocks('NEXUS').subscribe({
  ...
  getSlotEvent(hourStr: string): NexusApprovedEvent | null {
