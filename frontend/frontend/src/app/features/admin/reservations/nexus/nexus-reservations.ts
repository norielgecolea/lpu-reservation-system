...
import { NexusRescheduleCalendar, NexusRescheduleEvent } from './nexus-reschedule-calendar';
import { NexusCoordinationSlot, NexusCoordinationCalendar } from './nexus-coordination-calendar';
...
import { NexusReservationsService } from './nexus-reservations.service';
...
import { exportNexusReservationsCsv, ExportDateRange } from '../reservation-export.util';
...
import { downloadNexusReservationForm } from './nexus-reservation-form-export.util';
import { NexusEditDetailsModal } from './nexus-edit-details-modal';
...
interface NexusReservationViewRow extends NexusReservationRecord {
...
@Component({
  selector: 'app-nexus-reservations',
  imports: [ RouterLink, UiButton, UiIcon, UiInputSearch, UiToast, UiDateSelector, NexusRescheduleCalendar, NexusCoordinationCalendar, NexusEditDetailsModal, MaintenanceCalendarPicker, ReservationExportModal, ReservationStatusReasonModal, ApprovedReservationActionsMenu, ReservationApproverTableSkeleton, ReservationApproverMobileSkeleton, DashboardEventSummaryModal, ReservationApproverStatusChips, ReservationStatusPill],
  ...
  template: `
      ...
      <section class="animate-rise flex shrink-0 flex-col gap-3">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
              <h1 class="text-xl font-black tracking-tight text-gray-900">Nexus Room Reservations</h1>
    ...
    @if (coordinationTarget()) {
      <app-nexus-coordination-calendar
...
export class NexusReservations implements OnInit, OnDestroy {
  private readonly svc = inject(NexusReservationsService);
  ...
  protected readonly addReservationPath = adminAddReservationPath('nexus', this.router.url);

  readonly loading = signal(true);
  readonly apiError = signal(false);
  readonly reservations = signal<NexusReservationRecord[]>([]);
  ...
  protected readonly detailsSummaryEvent = computed(() => {
    ...
    return row ? reservationRecordToSummaryEvent(row, 'Nexus') : null;
  });
  ...
  readonly rescheduleApprovedEvents = computed<NexusRescheduleEvent[]>(() => {
  ...
  readonly coordinationCalendarEvents = computed<NexusRescheduleEvent[]>(() => {
  ...
  readonly coordinationInitialSlot = computed<NexusCoordinationSlot | null>(() => {
    const target = this.coordinationTarget();
  ...
  readonly filtered = computed((): NexusReservationViewRow[] => {
  ...
  ngOnInit(): void {
    ...
    this.wsSub = this.realtime.nexusUpdates$.subscribe(ev => this.handleWsEvent(ev));
    ...
    this.svc.getAll({ month: this.activeMonth() }).subscribe({
      next: (res) => {
        if (res.success) {
          ...
          this.alerts.watchPending('NEXUS', rows);
        } else {
  ...
  runExport(range: ExportDateRange): void {
    this.svc.getAll({ fromDate: range.startDate, toDate: range.endDate }).subscribe({
      next: (res) => {
        exportNexusReservationsCsv(res.reservations ?? [], range);
        this.exportOpen.set(false);
        this.toast.set('Nexus Room reservations exported to CSV');
      },
      error: () => {
  ...
  requestConfirm(row: NexusReservationRecord, action: ReservationStatus): void {
    this.confirm.set({ id: row.id, action, eventTitle: row.eventTitle });
  }

  openDetails(row: NexusReservationRecord): void { this.detailsTarget.set(row); }
  closeDetails(): void { this.detailsTarget.set(null); }

  async printForm(row: NexusReservationRecord): Promise<void> {
    try {
      await downloadNexusReservationForm(row);
    } catch {
  ...
  openEditDetails(row: NexusReservationRecord): void {
    if (!this.isSuperAdminRole()) return;
  ...
  saveEditDetails(body: NexusReservationDetailsEditRequest): void {
  ...
  openReschedule(row: NexusReservationRecord): void {
  ...
  loadMaintenance(): void {
    this.maintSvc.getBlocks('NEXUS').subscribe({
  ...
  addMaintenanceBlock(slot: MaintenanceSlot): void {
    ...
    this.maintSvc.createBlock({ facility: 'NEXUS', blockDate: slot.date, startTime: slot.startTime, endTime: slot.endTime, reason: slot.reason }).subscribe({
      next: (res) => {
