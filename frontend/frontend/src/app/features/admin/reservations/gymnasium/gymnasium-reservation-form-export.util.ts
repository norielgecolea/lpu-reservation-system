...
const FIELD = {
  ...
  gymnasiumCheck: { x: 192.5, y: PAGE_H - 257.6 + 1.2 },
...
function fillPage(page: PDFPage, font: PDFFont, row: GymReservationRecord): void {
  ...
  drawText(page, font, 'Gymnasium', FIELD.venue.x, FIELD.venue.y, 9, 300);
  drawText(
    page,
    font,
  ...
  drawCheck(page, font, FIELD.gymnasiumCheck.x, FIELD.gymnasiumCheck.y);
...
/** Fill the official IFERF PDF for a gymnasium reservation and download it. */
export async function downloadGymnasiumReservationForm(row: GymReservationRecord): Promise<void> {
  ...
  anchor.href = url;
  anchor.download = `IFERF-Gymnasium-${slugify(row.organization)}-${slugify(row.eventTitle)}.pdf`;
  anchor.click();
  URL.revokeObjectURL(url);
}

/** Build a gym form row from a dashboard summary event. */
export function gymFormRowFromDashboardEvent(event: DashboardEvent): GymReservationRecord {
  ...
  return {
    ...
    status: event.status as GymReservationRecord['status'],
    createdAt: '',
...
export async function downloadGymnasiumReservationFormFromEvent(
  event: DashboardEvent,
): Promise<void> {
  return downloadGymnasiumReservationForm(gymFormRowFromDashboardEvent(event));
}
