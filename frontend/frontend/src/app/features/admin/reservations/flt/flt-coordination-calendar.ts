          ...
          <div class="rounded-xl overflow-hidden ring-1 ring-black/5 shadow-sm bg-white">
            @for (slot of timeSlots; track slot.value) {
              @if (getBlockingEvent(slot.value); as ev) {
                <!-- Event / reservation — cannot overlap -->
                ...
                <!-- Free or existing coordination — selectable (coordination may overlap) -->
                <div
...
export class FltCoordinationCalendar {
  ...
  getBlockingEvent(hourStr: string): RescheduleEvent | null {
  ...
  getCoordinationOnSlot(hourStr: string): RescheduleEvent | null {
    const day = this.selectedDay();
    if (!day) return null;
    const hour = parseInt(hourStr, 10);
    return this.events.find(ev => {
  ...
  toggleHour(hourStr: string): void {
    ...
    if (start === null) {
      ...
      // Block overlap with events/reservations only — coordination may overlap
