...
import {
  FLT_EVENT_TYPES,
  FLT_ROOM_TYPES,
} from '../../../customer/flt/flt-reservation.models';
import {
...
@Component({
  selector: 'app-flt-edit-details-modal',
        ...
        <form class="flex flex-col gap-4 px-5 py-4" [formGroup]="form" (ngSubmit)="submit()">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <ui-select id="eventType" formControlName="eventType" placeholder="Select type" [options]="eventTypeOptions" />
            </div>
            <div class="flex flex-col gap-1.5">
              <label uiLabel for="roomType">Room <span class="text-red-500">*</span></label>
              <ui-select id="roomType" formControlName="roomType" placeholder="Select room" [options]="roomTypeOptions" />
            </div>
            <div class="flex flex-col gap-1.5">
            ...
            <div class="flex flex-col gap-1.5">
              <label uiLabel for="expectedAttendees">Expected Attendees <span class="text-red-500">*</span></label>
              <input uiInput id="expectedAttendees" type="number" min="1" formControlName="expectedAttendees" />
            </div>
            <div class="sm:col-span-2 flex flex-col gap-1.5">
...
export class FltEditDetailsModal {
  readonly reservation = input.required<FltReservationRecord>();
  readonly saving = input(false);
  readonly saved = output<FltReservationDetailsEditRequest>();
  readonly cancelled = output<void>();

  readonly error = signal('');
  readonly departmentOptions = DEPARTMENT_SELECT_OPTIONS;
  readonly eventTypeOptions = FLT_EVENT_TYPES.map((t) => ({ value: t.value, label: t.label }));
  readonly roomTypeOptions = FLT_ROOM_TYPES.map((r) => ({ value: r.value, label: r.label }));
  ...
  readonly form = new FormGroup({
    ...
    expectedAttendees: new FormControl(1, { nonNullable: true, validators: [Validators.required, Validators.min(1)] }),
  ...
  constructor() {
    effect(() => {
      ...
      this.error.set('');
      const attendees = Number.parseInt(String(row.expectedAttendees ?? '1'), 10);
      this.form.patchValue({
        ...
        expectedAttendees: Number.isFinite(attendees) && attendees > 0 ? attendees : 1,
  ...
  submit(): void {
    ...
    this.saved.emit({
      ...
      roomType: v.roomType,
      expectedAttendees: v.expectedAttendees,
