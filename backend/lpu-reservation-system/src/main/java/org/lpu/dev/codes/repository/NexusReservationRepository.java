...
@Repository
public class NexusReservationRepository {

    @PersistenceContext
    private EntityManager entityManager;

    public void save(NexusReservation reservation) {
        entityManager.persist(reservation);
        entityManager.flush();
    }

    public List<NexusReservation> findAllApproved() {
        return entityManager
                .createQuery("FROM NexusReservation r WHERE r.status IN ('APPROVED', 'COMPLETED') ORDER BY r.createdAt DESC", NexusReservation.class)
                .getResultList();
    }

    public Optional<NexusReservation> findById(Long id) {
        NexusReservation result = entityManager.find(NexusReservation.class, id);
        return Optional.ofNullable(result);
    }

    public boolean deleteById(Long id) {
        NexusReservation r = entityManager.find(NexusReservation.class, id);
        if (r == null) {
            return false;
        }
    ...
    public List<NexusReservation> findAllForConflictCheck() {
        return entityManager
                .createQuery(
                        "FROM NexusReservation r WHERE r.status IN ('PENDING', 'APPROVED', 'COMPLETED')",
                        NexusReservation.class)
                .getResultList();
    }

    public List<NexusReservation> findByStatus(String status) {
        return entityManager
                .createQuery("FROM NexusReservation r WHERE r.status = :status", NexusReservation.class)
    ...
    public void approve(Long id, String approvedBy) {
        entityManager.createNativeQuery(
                "UPDATE nexus_reservations SET status = 'APPROVED', approved_at = CURRENT_TIMESTAMP, approved_by = :approvedBy WHERE id = :id")
    ...
    public void updateStatusBatch(List<Long> ids, String status) {
        ...
        entityManager.createNativeQuery(
                "UPDATE nexus_reservations SET status = :status WHERE id IN (:ids)")
                .setParameter("status", status)
    ...
    public void updateCoordination(Long id, String date, String startTime, String endTime) {
        entityManager.createNativeQuery(
                "UPDATE nexus_reservations SET coordination_date = :date, " +
    ...
    public void reschedule(Long id, String reservedDatesJson) {
        entityManager.createNativeQuery(
                "UPDATE nexus_reservations SET reserved_dates = CAST(:json AS jsonb) WHERE id = :id")
    ...
    public void updateRating(Long id, int rating) {
        entityManager.createNativeQuery(
                "UPDATE nexus_reservations SET satisfaction_rating = :rating WHERE id = :id")
    ...
    @SuppressWarnings("unchecked")
    public List<NexusReservation> findApprovedByReservedDate(String date) {
        List<Number> ids = entityManager.createNativeQuery(
                "SELECT id FROM nexus_reservations WHERE status = 'APPROVED' "
        ...
        return entityManager
                .createQuery("FROM NexusReservation r WHERE r.id IN :ids", NexusReservation.class)
