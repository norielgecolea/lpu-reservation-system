...
public class NexusEmailService {

    private static final Logger logger = LogManager.getLogger(NexusEmailService.class);
    private static final String SERVICE_KEY = "nexus";
    private static final String SERVICE_LABEL = "Nexus Room";
    ...
    @Async
    public void sendReservationConfirmation(NexusReservation r) {
        String messageHtml =
            "<p style='color:#374151;font-size:15px;margin:0 0 12px;'>Your reservation is now <strong>pending review</strong> by the Nexus team. "
    ...
    @Async
    public void sendApprovalEmail(NexusReservation r) {
        String body = buildBase("Reservation Approved", "✅ Your reservation has been approved!", "#059669", r,
            "<p style='color:#374151;font-size:15px;margin:0 0 12px;'>Great news! The Nexus team has <strong>approved</strong> your reservation. "
    ...
    @Async
    public void sendRejectionEmail(NexusReservation r) {
    ...
    @Async
    public void sendConflictEmail(NexusReservation r) {
    ...
    @Async
    public void sendCancellationEmail(NexusReservation r) {
        String body = buildBase("Reservation Cancelled", "❌ Your reservation has been cancelled", "#6b7280", r,
            "<p style='color:#374151;font-size:15px;margin:0 0 12px;'>Your reservation has been <strong>cancelled</strong> by the Nexus administration. "
            + "You may submit a new reservation if you still need to use the facility.</p>", null);
        send(r.getContactEmail(), threadSubject(r), body, r.getId(), false);
    }

    @Async
    public void sendRescheduleEmail(NexusReservation r, String previousDatesJson) {
        ...
        String body = buildBase(
            ...
            + "The Nexus team has <strong>rescheduled</strong> your reservation. "
            ...
            + "If you have any questions about this change, please contact the Nexus office.</p>",
            null
        );
        send(r.getContactEmail(), threadSubject(r), body, r.getId(), false);
    }
    ...
    @Async
    public void sendCoordinationEmail(NexusReservation r, String coordDate, String coordStart, String coordEnd) {
        ...
        String body = buildBase("Coordination Meeting Scheduled", "📋 A coordination meeting has been set", "#d97706", r,
            "<p style='color:#374151;font-size:15px;margin:0 0 12px;'>The Nexus team has scheduled a <strong>coordination meeting</strong> for your upcoming event.</p>"
            + meetingInfo, null);
        send(r.getContactEmail(), threadSubject(r), body, r.getId(), false);
    }
     ...
     */
    public boolean sendReminderEmail(NexusReservation r, int daysBefore) {
    ...
    @Async
    public void sendSatisfactionSurvey(NexusReservation r) {
        String body = buildBase("Thank You", "Thank you for booking with us", "#7c3aed", r,
            ...
            + "Thank you for booking the LPU Laguna Nexus for your event. "
    ...
    private String buildSurveyStars(Long id) {
        ...
        for (int i = 1; i <= 5; i++) {
            String url = baseUrl + "/api/nexus/survey?id=" + id + "&rating=" + i;
    ...
    private String buildBase(String title, String headline, String accentColor,
                             NexusReservation r, String messageHtml, String extraHtml) {
        ...
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head>"
            ...
            + "<p style='margin:0;font-size:11px;font-weight:700;letter-spacing:2px;color:rgba(255,255,255,.7);text-transform:uppercase;'>LPU Laguna — Nexus</p>"
    ...
    private static String threadSubject(NexusReservation r) {
    ...
    private boolean send(String to, String subject, String htmlBody, Long reservationId, boolean threadRoot) {
        if (to == null || to.isBlank()) {
            logger.warn("Skipping nexus email — blank recipient for subject: {}", subject);
            return false;
        }
        try {
            ...
            mailSender.send(msg);
            logger.info("Nexus email sent to {} — {}", to, subject);
            return true;
        } catch (Exception e) {
            logger.error("Failed to send nexus email to {}: {}", to, e.getMessage(), e);
            return false;
        }
    }
}
